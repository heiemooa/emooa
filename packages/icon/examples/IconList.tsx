...
export default function ({ lang = 'zh-CN' }) {
  ...
  const iconItemStyle = name => ({
    ...
    aspectRatio: 3 / 2,
    cursor: 'pointer',
    background: hover === name ? '#efefef' : 'initial',
    transition: 'all .3s',
    borderRadius: 4,
  });
  const iconItemNameStyle = {
    marginTop: 10,
    overflow: 'hidden',
    whiteSpace: 'nowrap',
    ...
    <div>
      ...
      <Space style={spaceStyle}>
        ...
        <Button.Group>
          ...
          <Button onClick={() => setStrokeWidth(2)} type={strokeWidth === 2 ? 'primary' : 'secondary'}>
            2
          </Button>
          <Button onClick={() => setStrokeWidth(3)} type={strokeWidth === 3 ? 'primary' : 'secondary'}>
            3
          </Button>
          <Button onClick={() => setStrokeWidth(4)} type={strokeWidth === 4 ? 'primary' : 'secondary'}>
            4
          </Button>
            ...
            <div style={iconListStyle}>
              {filteredData.map(n => {
                ...
                return (
                  <div
                    key={n.componentName}
                    aria-label={n.componentName}
                    style={iconItemStyle(n.componentName)}
                    onMouseEnter={() => setHover(n.componentName)}
